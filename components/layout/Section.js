...
const Section = ({children, id, title, verticalCenter, moreLink, moreTitle}) => {
	return (
		<div id={id} className={`flex ${verticalCenter ? "h-screen items-center" : "pt-20"}`}>
			<div>
				<h2 className={"font-bold mb-10 inline-block"}>{title}</h2> <a
				className={"ml-3 text-primary hover:underline"} target={"_blank"} rel={"noreferrer noopener"}
				href={moreLink}>{moreTitle}</a>
				<div>
					{children}
				</div>
			</div>
